...
from mae_envs.modules.walls import RandomWalls,Centersquare
...
def make_env(n_substeps=5, horizon=250, deterministic_mode=False, n_agents=2,
             n_boxes=0, n_ramps=0, n_hiders=1, n_seekers=1, other_friction=0.01, gravity=[0, 0, -50],
             polar_obs= False, start_seed=None, cone_angle=3/8 * np.pi, placement_fn=None):
    '''
    ...
    env = Base(n_agents=n_agents, n_substeps=n_substeps, horizon=horizon,
               deterministic_mode=deterministic_mode, start_seed=start_seed)
    ...
    env = AgentAgentObsMask2D(env, cone_angle=cone_angle)
