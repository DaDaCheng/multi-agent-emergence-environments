...
def rejection_placement(env, placement_fn, floor_size, obj_size, num_tries=10):
    ...
    for i in range(num_tries):
        ...
        if placement_fn is not None:
            ...
            #pos = placement_fn(grid, obj_size_in_cells, env.metadata, np.random.RandomState(1))

            #print(env._random_state)
        else:
            # Assume that we'll always have boundary walls so don't sample there
            rr1=np.random.RandomState(1+i)
            ...
            pos = np.array([rr1.randint(1, grid_size - obj_size_in_cells[0] - 1),
                           rr1.randint(1, grid_size - obj_size_in_cells[1] - 1)])
